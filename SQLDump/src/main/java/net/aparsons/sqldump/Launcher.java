...
public class Launcher {
	 ...
	 *  All the options contains an argument, with the exception of includeHeaders   
	 * @return Available command line options
	 */
	public static Options getOptions() {
		final Options options = new Options();
		
		final Option url = new Option("url", true, "A database url of the form jdbc:subprotocol:subname");
		...
		OptionGroup mandatory = new OptionGroup().addOption(url)
													.addOption(user)
													.addOption(pass)
													.addOption(query);
	...
	public static String printUsage() {
		...
		return "java -jar SQLDump-0.4.jar -url [jdbc:oracle:thin:@hostname:port:sid] -user [username] -pass [password] -sql [query]";
	}
	 ...
	 * The hash map must contain all the required paramenter. If a mandatory one is missed, it prints the parameter usage and throws an exception. 
	 * Use the parameter names defined in getOption method.
	 * @param consoleParams Command line arguments
	 * @throws Exception 
	 */
	public static Map<String, String> parse(String[] consoleParams) throws Exception {
		...
		try {
			...
			if(!flag && cmdLine.hasOption("file"))
